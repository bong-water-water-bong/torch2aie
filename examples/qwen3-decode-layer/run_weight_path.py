...
"""Runner for the isolated Qwen3 Q4NX weight-path microbench."""
...
os.environ.setdefault("QWEN3_MAIN16_KERNEL_SOURCE", "qwen3_decode_weight_path.cc")
...
from cases import weight_path_microbench_generate as generate
...
BUILD_NAME = "qwen3-8b-weight-path"
...
def build_kernel() -> tuple[Path, Path]:
    ...
    if errors:
        raise RuntimeError("\n".join(f"  QWEN3 WEIGHT-PATH STRUCTURE FAIL: {error}" for error in errors))
...
def run(
    ...
    print("=" * 78)
    print(f"qwen3-layer isolated weight-path microbench: {generate.CASE_NAME}")
    print("=" * 78)
    print(f"  model={_model_path(model_path)}")
    print(f"  layer={layer}")
    print("  topology=weights BO -> shim MM2S -> memtile patch ping/pong -> row streams -> Main16 DMA1")
    print("  numerics=disabled; no activation, record, attention, or FFN path is generated")
    print()
    ...
    print("  Preparing real aux-prefixed Q4NX weight stream...")
    ...
    result = npu_build.run(handle, [weights_buf])
    ...
    print("  PASS: isolated weight-path microbench completed")
    return True


def main() -> None:
    parser = argparse.ArgumentParser(prog="qwen3 isolated weight-path microbench runner")
