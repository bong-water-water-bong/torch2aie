...
// mm.cc provides: matmul_bf16_bf16, zero_bf16, etc.
#include "mm.cc"
                     ...
                     int32_t S_kv_eff)
{
    ...
    for (; i + 4 <= valid_q_rows; i += 4) {
        int32_t valid_cols0 = valid_kv_cols;
        int32_t valid_cols1 = valid_kv_cols;
        int32_t valid_cols2 = valid_kv_cols;
        int32_t valid_cols3 = valid_kv_cols;
        if (kv_block_idx == q_block_idx) {
            valid_cols0 = (valid_cols0 < i + 1) ? valid_cols0 : i + 1;
            valid_cols1 = (valid_cols1 < i + 2) ? valid_cols1 : i + 2;
            valid_cols2 = (valid_cols2 < i + 3) ? valid_cols2 : i + 3;
            valid_cols3 = (valid_cols3 < i + 4) ? valid_cols3 : i + 4;
        }
        partial_softmax_masked64_alias_bf16(A + B_kv * i, P + B_kv * i, scale_buffer, i, B_q, inv_scale, valid_cols0);
        partial_softmax_masked64_alias_bf16(
            A + B_kv * (i + 1), P + B_kv * (i + 1), scale_buffer, i + 1, B_q, inv_scale, valid_cols1);
        partial_softmax_masked64_alias_bf16(
            A + B_kv * (i + 2), P + B_kv * (i + 2), scale_buffer, i + 2, B_q, inv_scale, valid_cols2);
        partial_softmax_masked64_alias_bf16(
            A + B_kv * (i + 3), P + B_kv * (i + 3), scale_buffer, i + 3, B_q, inv_scale, valid_cols3);
    }
    for (; i < valid_q_rows; i++) {
        int32_t valid_cols = valid_kv_cols;
        if (kv_block_idx == q_block_idx) {
            valid_cols = (valid_cols < i + 1) ? valid_cols : i + 1;
        }
        partial_softmax_masked64_alias_bf16(A + B_kv * i, P + B_kv * i, scale_buffer, i, B_q, inv_scale, valid_cols);
    }
