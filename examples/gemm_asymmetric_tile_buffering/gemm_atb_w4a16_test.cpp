...
//===----------------------------------------------------------------------===//
//
// Host runner for the W4A16 ATB GEMM config in this folder. A and C are
// v8bfp16ebs8 on device. B is sent as a compact Q4NX-like L1 tile packet:
// bf16 scales, bf16 zero-points, and packed 4-bit weights with group_size=32.
// The AIE core dequantizes each B L1 tile into local BFP16 once, then reuses
// the existing high-throughput ATB BFP16 microkernel for the six A sub-tiles.
//
// This is otherwise identical to ../matrix_multiplication/bfp_test.cpp except
// for the ATB-specific layouts:
//   - A is pre-shuffled via gemm_atb::layout_A_L1_2x1_8x8block,
//   - B is packed as W4A16 metadata plus int4 payload in the same ATB stream
//     order that gemm_atb::layout_transpose_L1_1x2_8x8block would produce,
...
int main(int argc, const char *argv[]) {
  ...
  options.add_options()("trows,w", "Tile size m",
      ...
      "w4-group-size", "W4A16/AWQ quantization group size along K",
      cxxopts::value<int>()->default_value("32"))(
      "w4-symmetric", "use signed symmetric W4 instead of asymmetric AWQ",
  ...
  int k = vm["y"].as<int>();
  int n = vm["z"].as<int>();
  int w4_group_size = vm["w4-group-size"].as<int>();
  bool w4_symmetric = vm["w4-symmetric"].as<bool>();

  if (w4_group_size != 32) {
    std::cerr << "config2_w4a16 is compiled for Q4NX/AWQ group_size=32."
              << std::endl;
    return 1;
  }
  ...
  size_t B_VOLUME =
      static_cast<size_t>(K / k) * static_cast<size_t>(N / n) *
      gemm_atb::w4a16_atb_l1_packet_bytes(k, n, w4_group_size);
  ...
  std::vector<float> BVecSource(B_SIZE);
  for (int i = 0; i < B_SIZE; i++) {
    BVecSource[i] = 1.0f;
  }

  auto shuffleStart = std::chrono::high_resolution_clock::now();
  // ATB: A uses row-major L1 tiles with a 2x1/8x8 sub-block pattern and is
  // BFP-encoded on host. B is quantized into group-32 W4A16 packets in the
  // matching 1x2/8x8 ATB stream order, then dequantized on core.
  ...
  gemm_atb::W4A16AtbL1Packets BVecW4 =
      gemm_atb::pack_w4a16_atb_l1_packets(BVecSource, K, N, k, n,
                                          w4_group_size, w4_symmetric);
  std::vector<float> &BVec = BVecW4.dequantized_row_major;
  assert(B_VOLUME == BVecW4.bytes.size() &&
         "host-computed W4 buffer size must match packet helper");
  ...
  memcpy(bufB, BVecW4.bytes.data(), B_VOLUME);
  ...
  std::cout << "Avg NPU tflops: "
            << macs / (1000000 * npu_time_total / n_iterations) << std::endl;

  std::cout << std::endl
            << "Min NPU matmul time: " << npu_time_min << "us." << std::endl;
  std::cout << "Max NPU gflops: " << macs / (1000 * npu_time_min) << std::endl;
  std::cout << "Max NPU tflops: " << macs / (1000000 * npu_time_min) << std::endl;

  std::cout << std::endl
            << "Max NPU matmul time: " << npu_time_max << "us." << std::endl;
  std::cout << "Min NPU gflops: " << macs / (1000 * npu_time_max) << std::endl;
  std::cout << "Min NPU tflops: " << macs / (1000000 * npu_time_max) << std::endl;

  std::cout << std::endl
            << "Shuffle time: "
            << inputShuffleTime + (outShuffleTime / n_iterations) << "us."
            << std::endl;
  std::cout << "W4A16/Q4NX-like B: group_size=" << w4_group_size
            << ", symmetric=" << (w4_symmetric ? "true" : "false")
            << ", l1_packet_bytes=" << BVecW4.packet_bytes_per_l1
            << ", total_weight_bytes=" << B_VOLUME << std::endl;
