...
def my_matmul(M, K, N, m, k, n):
    ...
    trace_size = 8192
    w4_group_size = 32
    w4_groups_per_l1_k = k // w4_group_size
    w4_metadata_values_per_l1 = w4_groups_per_l1_k * n
    w4_l1_packet_bytes = w4_metadata_values_per_l1 * 2 * 2 + (k * n) // 2

    # Use BFP16 for activations/output and Q4NX-like W4 packets for weights.
    ...
    def device_body():
        ...
        B_l2_ty = np.ndarray[
            (w4_l1_packet_bytes,), np.dtype[np.uint8]
        ]  # W4 packed weights plus bf16 scale/zero metadata.
        C_l2_ty = np.ndarray[(n_aie_rows * m, n // 8), np.dtype[dtype_out]]

        A_l1_ty = np.ndarray[(m // DIV, k // 8), np.dtype[dtype_in]]
        B_l1_ty = np.ndarray[
            (w4_l1_packet_bytes,), np.dtype[np.uint8]
        ]  # One compact W4A16 L1 B tile.
        ...
        @runtime_sequence(
            ...
            np.ndarray[
                ((K // k) * (N // n) * w4_l1_packet_bytes,), np.dtype[np.uint8]
            ],
        ...
        def sequence(A, B, C):
            ...
            B_taps = TensorTiler2D.group_tiler(
                (1, (N // n) * (K // k) * w4_l1_packet_bytes),
                (1, (K // k) * w4_l1_packet_bytes),
                (1, 1),
            )
